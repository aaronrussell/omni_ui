defmodule Omni.UI.Files.PlugTest.Endpoint do
  @moduledoc false
  def config(:secret_key_base), do: String.duplicate("a", 64)
end

defmodule Omni.UI.Files.PlugTest do
  ...
  alias Omni.UI.Files.Plug, as: FilePlug
  alias Omni.UI.Files.PlugTest.Endpoint
  ...
  defp call_plug(conn, plug_opts \\ []) do
    opts = FilePlug.init(plug_opts)
    FilePlug.call(conn, opts)
  end
  ...
  describe "successful serving" do
    ...
    test "file_url encodes special characters in filename" do
      url = URL.file_url(Endpoint, "sess", "my app.html")
  ...
  describe "authentication" do
    ...
    test "returns 401 for expired token", %{fs: fs} do
      FS.write(fs, "page.html", "<h1>Hello</h1>")
      token = Phoenix.Token.sign(Endpoint, "omni_ui:file", @session_id, signed_at: 0)
