defmodule Omni.UI.MacroTest do
  ...
  describe "public API" do
    test "init_session/2 is exported" do
      assert function_exported?(Omni.UI, :init_session, 2)
    end

    test "attach_session/2 is exported" do
      assert function_exported?(Omni.UI, :attach_session, 2)
    end

    test "ensure_session/1 is exported" do
      assert function_exported?(Omni.UI, :ensure_session, 1)
    end

    test "update_session/2 is exported" do
      assert function_exported?(Omni.UI, :update_session, 2)
    end

    test "notify/2 and notify/3 are exported" do
      assert function_exported?(Omni.UI, :notify, 2)
      assert function_exported?(Omni.UI, :notify, 3)
    end
  end

  describe "init_session/2" do
    test "populates every Omni.UI-owned assign" do
    ...
    test ":manager defaults to Omni.UI.Sessions" do
      socket = Omni.UI.init_session(build_socket(), model: {:anthropic, "claude-haiku-4-5"})

      assert socket.assigns.manager == Omni.UI.Sessions
    end

    test ":manager honours an explicit override" do
      socket =
        Omni.UI.init_session(build_socket(),
    ...
    test "raises ArgumentError on an unresolvable model ref" do
      assert_raise ArgumentError, ~r/failed to resolve model/, fn ->
        Omni.UI.init_session(build_socket(), model: {:nope, "nothing"})
      end
    end
  ...
  describe "update_session/2 (no session attached)" do
    defp init_socket do
      Omni.UI.init_session(build_socket(), model: {:anthropic, "claude-haiku-4-5"})
    end

    test "updates :model when ref resolves" do
      socket = Omni.UI.update_session(init_socket(), model: {:anthropic, "claude-sonnet-4-5"})
    ...
    test "ignores an unresolvable model ref and warns the user" do
      ...
      socket = Omni.UI.update_session(socket, model: {:nope, "nothing"})
    ...
    test "updates :thinking" do
      socket = Omni.UI.update_session(init_socket(), thinking: :high)

      assert socket.assigns.thinking == :high
    end

    test "updates :system" do
      socket = Omni.UI.update_session(init_socket(), system: "be helpful")
    ...
    test "updates :tools and :tool_components together" do
      ...
      socket = Omni.UI.update_session(init_socket(), tools: [{tool_a, component: renderer}])
  ...
  describe "normalise_tools/1" do
    ...
    test "returns empty list and empty map for empty input" do
      assert {[], %{}} = Omni.UI.normalise_tools([])
    end
    ...
    test "passes bare structs through without adding to components map" do
      ...
      assert {[^a, ^b], %{}} = Omni.UI.normalise_tools([a, b])
    end
    ...
    test "extracts component from tuple entries" do
      ...
      assert {[^a], %{"a" => ^fun}} = Omni.UI.normalise_tools([{a, component: fun}])
    end
    ...
    test "handles a mixed list of bare structs and tuples" do
      ...
      assert {[^a, ^b, ^c], components} =
               Omni.UI.normalise_tools([{a, component: fun_a}, b, {c, component: fun_c}])
