defmodule Omni.UI.SessionLifecycleTest do
  use Omni.UI.SessionCase, async: true
  ...
  # `Omni.UI.Sessions` while keeping the test isolated.
  ...
  defp init_socket do
    Omni.UI.init_session(build_socket(), manager: TestManager, model: model())
  end
  ...
  describe "attach_session/2" do
    test "attaches an existing session and applies its snapshot to the socket" do
      ...
      socket = Omni.UI.attach_session(init_socket(), id: id)
    ...
    test "raises when the id is not found in the store" do
      assert_raise RuntimeError, ~r/Omni.Session .* not found/, fn ->
        Omni.UI.attach_session(init_socket(), id: "does-not-exist")
      end
    end
    ...
    test "id: nil resets the socket to a blank session state" do
      ...
      socket =
        init_socket()
        |> Omni.UI.attach_session(id: id)
        |> Omni.UI.attach_session(id: nil)
    ...
    test "is a no-op when the same id is reattached" do
      ...
      attached = Omni.UI.attach_session(init_socket(), id: id)
      reattached = Omni.UI.attach_session(attached, id: id)
