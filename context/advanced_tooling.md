...
23. **REPL tool-use component ✓** — `OmniUI.REPL.ChatUI.tool_use/1` replaces the default renderer entirely (unlike `Artifacts.ChatUI` which wraps it). Terminal icon instead of cog, agent-provided `title` in the toggle instead of tool name, syntax-highlighted Elixir code in the expanded body instead of raw JSON params. Wired via `{OmniUI.REPL.Tool.new(...), component: &OmniUI.REPL.ChatUI.tool_use/1}` in `create_tools/1`. Did not require a `:toggle` override on `Components.tool_use/1` — the component uses `expandable/1` directly with its own `:toggle` slot content.
24. **Panel visibility toggle ✓** — `@view_artifacts` boolean assign controls panel visibility. Header component with a toggle button (icon swaps between `panel_right_open` / `panel_right_close`). Chat area expands to full width when panel is hidden. `view_artifact` event auto-opens the panel. See Open Question #3.
25. **Error handling ✓** — `PanelComponent.update({:view, filename})` now detects missing artifacts (stale view buttons from earlier in the session where the artifact was later deleted). Returns to the list view with an `@error` assign. `artifact_list/1` renders the error as an inline notice above the list. Error clears automatically on the next successful view, artifact selection, or rescan.
...
### 3. Panel auto-open behaviour ✓ (Resolved)

**Decision:** Hybrid approach — manual toggle + auto-open on view. The panel is hidden by default (`@view_artifacts: false`). A header toggle button lets the user show/hide the panel manually. The `view_artifact` event (triggered by inline "View" buttons in chat) auto-opens the panel by setting `@view_artifacts: true`. No notification badge or auto-open on first creation — the user controls visibility, with the convenience of auto-open when they explicitly ask to view an artifact.
